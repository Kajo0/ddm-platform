...
class Svm2LvlTrainTestFashionMnistSpec extends Specification {
    ...
    def "should #l_method #filtering_method - 1st=#first_lvl scatter data among files and perform correct classifications of training data"() {
        ...
        def idIndex = 0
        def labelIndex = 1
        def attributesAmount = 784
        def classes = 10

        def trainFile = Paths.get(getClass().getResource('/fashion-mnist_train.csv').path)
        def testFile = Paths.get(getClass().getResource('/fashion-mnist_test.csv').path)
        ...
        def strategies = [
                ...
                'all-but'              : 'fillEmptyButPercent=0.6;additionalClassesNumber=-8;additionalClassesPercent=0.05;emptyWorkerFill=1',
        ]
        ...
        def dataDesc = new PartitionerStrategy.DataDesc(
                ...
                separator: separator,
                filesLocations: [trainFile.toFile().path]
        )
        ...
        def config = DdmExecutionConfig.builder()
                ...
                .dataPath(dataPaths)
                .testDataPath(testFile.toFile().path)
                .separator(separator)
                .idIndex(idIndex)
                .labelIndex(labelIndex)
