...
class Svm2LvlTrainTestShuttleSpec extends Specification {
    ...
    def "should #l_method #filtering_method - 1st=#first_lvl scatter data among files and perform correct classifications of training data"() {
        ...
        def idIndex = 0
        def labelIndex = 10
        def attributesAmount = 9

        def trainFile = Paths.get(getClass().getResource('/shuttle.trn').path)
        def testFile = Paths.get(getClass().getResource('/shuttle.tst').path)
